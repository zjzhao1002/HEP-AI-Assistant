...
Then you can chat with the agent. MCP tools are not enabled in the hosted Streamlit app.
...
The Streamlit app in the Docker image does not enable MCP tools.
...
4.  **Optionally configure MCP servers for CLI/source-agent runs:**
    ...
    Restart the application after changing the MCP configuration or MCP skills. MCP tools are currently disabled in `streamlit_app_local.py`, `streamlit_app_cloud.py`, and the Docker Streamlit app.
...
After changing sidebar settings, click **Save configuration** before chatting so the app reloads the agent with the selected environment and model values. The local app sets `ARXIVFLOW_KEYWORD_BACKEND="ollama"` automatically, so trend recommendations use the configured Ollama model. MCP tools are disabled in the local Streamlit app for now.
...
The cloud app sets `ARXIVFLOW_KEYWORD_BACKEND="gemini"` automatically, does not expose Ollama or MCP settings, and uses `GOOGLE_API_KEY` directly for arXivFlow keyword extraction and Google Search fallback. PDFs generated by the `download_pdf` tool are stored in a temporary server directory and shown as in-browser download buttons so users can save them to their own device. When Markdown extraction succeeds, the app also keeps a `.md` sidecar next to the downloaded PDF and a temporary Chroma index for agent-side reading, paper-analysis, and local-context Q&A workflows. PDG particle-property and decay queries are available through the same chat interface.
